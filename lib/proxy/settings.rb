...
module Proxy::Settings
  ...
  def self.initialize_global_settings(settings_path = nil, argv = ARGV)
    ::Proxy::Settings::Global.new(YAML.load_file(settings_path || SETTINGS_PATH))
  end

  def self.load_plugin_settings(defaults, settings_file, settings_directory = nil)
    settings = {}
    begin
      settings = YAML.load_file(File.join(settings_directory || ::Proxy::SETTINGS.settings_directory, settings_file)) || {}
    rescue Errno::ENOENT
  ...
  def self.read_settings_file(settings_file, settings_directory = nil)
    YAML.load_file(File.join(settings_directory || ::Proxy::SETTINGS.settings_directory, settings_file)) || {}
  end
end
