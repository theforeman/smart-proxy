class Proxy::PuppetLegacy::PuppetConfigEnvironmentsRetriever < Proxy::Puppet::EnvironmentsRetrieverBase
  include ::Proxy::Log
  attr_reader :puppet_configuration, :puppet_config_file_path

  def initialize(puppet_configuration, puppet_config_file_path)
    @puppet_configuration = puppet_configuration
    @puppet_config_file_path = puppet_config_file_path
  end
