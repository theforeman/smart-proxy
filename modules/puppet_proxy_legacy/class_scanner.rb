module Proxy::PuppetLegacy
  class ClassScanner < ClassScannerBase
    attr_reader :puppet_initializer

    def initialize(environments_retriever, puppet_initializer)
      @puppet_initializer = puppet_initializer
    ...
    def scan_manifest manifest, filename = ''
      ...
      puppet_initializer.reset_puppet
      ...
      hostclasses.each do |klass|
        ...
        if klass.type == :hostclass && !already_seen.include?(klass.namespace)
          ...
          klasses << ::Proxy::Puppet::PuppetClass.new(klass.namespace, params)
        end
      end
      klasses
    rescue => e
