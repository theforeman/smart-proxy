...
class PuppetApiv3ClassesRetrieverTest < Test::Unit::TestCase
  ...
  def setup
    ...
    @retriever = Proxy::Puppet::PuppetApiV3ClassesRetriever.new(nil, nil, nil, nil, @api)
  end

  def test_uses_puppet_resource_api
    Proxy::Puppet::ResourceTypeApiv3.any_instance.expects(:list_classes).with('test_environment', 'class').returns([])
    Proxy::Puppet::PuppetApiV3ClassesRetriever.new(nil, nil, nil, nil).classes_in_environment('test_environment')
  end
  ...
  def test_should_raise_environment_not_found_when_puppet_cannot_find_it
    ...
    assert_raises Proxy::Puppet::EnvironmentNotFound do @retriever.classes_in_environment('test_environment') end
  end
  ...
  def test_should_re_raise_exception_on_other_errors
    ...
    assert_raises Proxy::Error::HttpError do @retriever.classes_in_environment('test_environment') end
  end
